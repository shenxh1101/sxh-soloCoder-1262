...
import { Settings2, Contrast, Maximize, FlipHorizontal, Sparkles } from "lucide-react";
...
const ControlPanel = () => {
  ...
  const isPresetActive = (presetId: string) => {
    ...
    return Object.entries(preset.params).every(
      ([key, value]) => params[key as keyof typeof params] === value
    );
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-terminal-bgDark/50 border border-terminal-border rounded-xl p-5 backdrop-blur-sm">
        <h3 className="text-terminal-green font-semibold flex items-center gap-2 mb-4">
          <Sparkles size={18} className="text-terminal-amber" />
          预设效果
        </h3>
        <div className="grid grid-cols-4 gap-2">
          {presets.map((preset) => (
              ...
              className={`
                ...
                ${
                  isPresetActive(preset.id)
                    ? "border-terminal-amber bg-terminal-amber/10 shadow-glowAmber"
            ...
            >
              ...
              <span className="text-[10px] text-terminal-green/80 font-medium">
                {preset.name}
              </span>
            </button>
          ))}
